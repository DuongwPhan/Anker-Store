...
function xshop_new_optins_texts() {
	global $pagenow;
    if( get_option('xshopfit4' ) || $pagenow == 'themes.php' ){
        return;
    }
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __( '<strong> Hi Buddy!! Now You are using the Free version of XSHOP theme.<br> XSHOP PRO version now avilable!! If you want a beautiful, orderly, SEO friendly, more secure and sell anything with WooCommerce then need to upgrade pro!! So Why You late? Build Your Site With Pro Version.</strong> ', 'xshop' );
...
function xshop_new_optins_texts_init(){
    if(isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1 ){
		delete_option( 'xshopfit2' );
        update_option( 'xshopfit4', 1);
    }
}
